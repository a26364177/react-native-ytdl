...
exports.getBasicInfo = async (id, options) => {
  ...
  const setIdentityToken = async key => {
    let token = await exports.cookieCache.getOrSet(key, async() => {
      let watchPageBody = await fetch(watchPageURL, {...reqOptions, credentials:'omit'}).then(body => body.text());
  ...
  let body = await fetch(jsonEndpointURL, {...reqOptions, credentials: 'omit'}).then(body => body.text());
  let info;
  try {
  ...
  if (!info.player) {
    ...
    body = await fetch(embedUrl, {...options.reqOptions, credentials: 'omit'}).then(body => body.text());
...
const gotConfig = async (id, options, info, body) => {
  ...
  if (!player_response) {
    ...
    let morebody = await fetch(url, {...options.requestOptions, credentials: 'omit'}).then(body => body.text());
...
const getM3U8 = async (url, options) => {
  ...
  let body = await fetch(url, {...options.requestOptions, credentials: 'omit'}).then(body => body.text())
  let formats = {};
  body
    .split('\n')
